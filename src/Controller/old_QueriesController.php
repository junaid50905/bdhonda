...
class QueriesController extends AppController
{
    ...
    public function index($product_slug = null)
    {
        if($this->request->is('post')){
            ...
            // Ensure $product_slug is sanitized and secure
            $slug = h(strip_tags($product_slug));
            $productTable = $this->getTableLocator()->get('Products');

            // Fetch product details along with associated colors
            $productDetails = $productTable->find()
            ->where(['Products.slug' => $slug])
            ->contain(['Colors']) // Load related colors based on product_id
            ->first();
        ...
        //     $this->set(compact('slug', 'productDetails'));
        //     $this->set('divisions', $this->Divisions->find('list')->toArray());
        // }
    }
